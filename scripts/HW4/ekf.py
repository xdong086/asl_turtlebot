...
class Ekf(object):
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        s_t = H.dot(self.Sigma).dot(H.T) + Q
        k_t = self.Sigma.dot(H.T).dot(np.linalg.pinv(s_t))
        self.x = self.x + k_t.dot(z.flatten())
        self.Sigma = self.Sigma - k_t.dot(s_t).dot(k_t.T)
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(z_raw.shape[1]):
            ...
            for j in range(hs.shape[1]):
                v_ij = np.array([[angle_diff(z_raw[0][i], hs[0][j])], [z_raw[1][i] - hs[1][j]]])
                innovation.append(v_ij)
                s_ij = Hs[j].dot(self.Sigma).dot(Hs[j].T) + Q_raw[i]
                d_ij = v_ij.T.dot(np.linalg.pinv(s_ij)).dot(v_ij)
                m_dist.append(d_ij)
            j_min = np.argmin(m_dist)
            if (m_dist[j_min] < self.g ** 2):
                v_list.append(innovation[j_min].reshape(2))
                Q_list.append(Q_raw[i])
                H_list.append(Hs[j_min])
                
            """
            S = Hs[j_min].dot(self.Sigma).dot(Hs[j_min].T) + Q_raw[i]
            K = self.Sigma.dot(Hs[j_min].T).dot(np.linalg.pinv(S))
            v = np.array([[angle_diff(z_raw[0][i], hs[0][j_min])], [z_raw[1][i] - hs[1][j_min]]])
            self.x = self.x + K.dot(v).reshape(-1)
            tmp = K.dot(Hs[j_min])
            self.Sigma = (np.eye(tmp.shape[1]) - tmp).dot(self.Sigma)
            """
...
class EkfSlam(Ekf):
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx[:,:3] = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[:3], self.tf_base_to_camera)
            ...
            if j >= 2:
                ...
                Hx[1,idx_j] = - np.sin(pos_angle - alpha) * np.sqrt(x_cam ** 2 + y_cam ** 2)
            ########## Code ends here ##########
